...
# b"Let's go to the Proven\xc3\xa7e!" | \pause |
\end{lstlisting}
    \item Here, \texttt{\textbackslash xc3} and \texttt{\textbackslash xa7} are \href{https://docs.python.org/3/reference/lexical_analysis.html#grammar-token-stringescapeseq}{escape sequences} that together encode \c{c} as a hex number (see \href{http://www.ltg.ed.ac.uk/~richard/utf-8.cgi?input=&mode=char}{UTF-8} tool) 
        \pause
...
\frametitle{What is a regular expression?}
    \begin{itemize}
        ...
        \item Made available in the \texttt{re} module \pause
        \item Specifies the rules to match (and replace) occurrences of patterns in text \pause 
        \item An example:
\begin{lstlisting}[style=python]
import re  | \pause |
str_to_match = "Factor levels are 1foo, 2foo and foo" | \pause |
# regex that matches '1foo', '2foo' etc but not 'foo'
regex = r"\dfoo" | \pause |  
...
re.search(regex, str_to_match) # Match object  | \pause |

regex = r"\dfAA" # another regex which does not match | \pause |  
...
\end{lstlisting} \pause
        ...
        \item Group contents can be recalled in the replacement, using \texttt{\textbackslash 1},\texttt{\textbackslash 2} 
    \end{itemize}
\end{frame}
%=============================================================================%
%=============================================================================%
...
m = re.search(regex, str_to_match) # Factor levels are snafoo, foosna and bar
m.group(0) # obtain the complete match from the Match object
